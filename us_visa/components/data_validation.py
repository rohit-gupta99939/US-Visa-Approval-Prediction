...
class DataValidation:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            logging.info("Entering initiate_data_validation method")
            ...
            logging.info(f"Number of columns present in training dataset : {status}")

            if not status:
                validation_error_msg += f"columns are missing from training dataframe."

            status = self.validate_number_of_columns(dataframe=test_df)
            logging.info(f"All required columns are present in testing dataframe.{status}")
            if not status:
                validation_error_msg += f"columns are missing from testing dataframe"
